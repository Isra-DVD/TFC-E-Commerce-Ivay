...
public class OrderServiceImpl implements OrderService {
	...
	public List<OrderResponseDto> getOrdersByUserId(Long userId) {
		...
		return orderRepository.findByUser_Id(userId).stream().map(orderMapper::toOrderResponse).toList();
	}
	...
	public OrderResponseDto createOrder(CreateOrderRequestDto createOrderRequestDto) {
		...
		order.setGlobalDiscount(
				createOrderRequestDto.getGlobalDiscount() != null ? createOrderRequestDto.getGlobalDiscount()
						: BigDecimal.ZERO);
		...
		for (CreateOrderItemRequestDto itemDto : createOrderRequestDto.getItems()) {
			...
			if (product.getStock() == null || product.getStock() < quantity) {
				log.error("Insufficient stock for product id {} ({}). Requested: {}, Available: {}", product.getId(),
			...
			BigDecimal itemTotal = product.getPrice().multiply(BigDecimal.valueOf(quantity))
					.multiply(BigDecimal.ONE.subtract(orderItem.getDiscount())).setScale(2, RoundingMode.HALF_UP);
	...
	public OrderResponseDto updateOrder(Long orderId, UpdateOrderDto updateOrderDto) {
		...
		if (updateOrderDto.getGlobalDiscount() != null
			...
			log.warn("Updating global discount for order {}. Recalculation of totalAmountDiscounted might be needed!",
					orderId);
			order.setGlobalDiscount(updateOrderDto.getGlobalDiscount());
			BigDecimal totalAmountDiscounted = order.getTotalAmount()
					.multiply(BigDecimal.ONE.subtract(order.getGlobalDiscount())).setScale(2, RoundingMode.HALF_UP);
			order.setTotalAmountDiscounted(totalAmountDiscounted);
			updated = true;
		}
