                ...
                for (const item of itemsFromApi) {
                    ...
                    if (item.productId && (!item.product || !item.product.id)) {
                        try {
                            ...
                            processedItem.product = productDetails;
                        } catch (prodError) {
                            console.error(
                                `Error fetching product details for ID ${item.productId}:`,
                                prodError
                            );
                            processedItem.product = null;
                        }
                    }
                    itemsWithProductDetails.push(processedItem);
                }
    ...
    const handleQuantityChange = async (cartItemId, newQuantity) => {
        ...
        try {
            ...
            if (originalItem && originalItem.product) {
                ...
                console.warn(
                    "Original item or product details not found for quantity update. Re-fetching cart."
                );
                fetchCartData();
                setActionMessage({
                    type: "warning",
                    text: "Cantidad actualizada. Refrescando carrito...",
                });
            }
        } catch (e) {
