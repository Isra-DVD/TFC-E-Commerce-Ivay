...
const CartItemRow = ({ item, onQuantityChange, onRemoveItem }) => {
  ...
  if (!product) {
    return (
      ...
      >
        Error: Datos del producto no disponibles para un artículo del carrito
        (ID Artículo: {item.id}, ID Producto: {item.productId || "N/A"}).
      </Typography>
    );
  }
  ...
  const itemSubtotal =
    product.price * (1 - (product.discount || 0)) * item.quantity;
