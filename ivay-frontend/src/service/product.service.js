...
const findProductsByName = async (name) => {
    const response = await axios.get(`${PRODUCT_API_URL}/filter`, { params: { name } });
    return response.data.data;
};
...
const updateProduct = async (productId, productRequestDto) => {
    const response = await axios.put(`${PRODUCT_API_URL}/${productId}`, productRequestDto);
    return response.data.data;
};
...
const getOrderItemsByProductId = async (productId) => {
    const response = await axios.get(`${PRODUCT_API_URL}/${productId}/order-items`);
    return response.data.data;
};

const getCartItemsByProductId = async (productId) => {
    const response = await axios.get(`${PRODUCT_API_URL}/${productId}/cart-items`);
    return response.data.data;
};
